...
public class TicketRestrict implements RestrictTypeFactory<Ticket>{

	@Override
	public CompletableFuture<RestrictType<Ticket>> create(DataFetchingEnvironment env) {
		...
		if (context.getMembership() == null) {
			...
			return context.getMembership().getRole(context).thenApply(role -> {
				return role.ticketAccess(context);
			});
